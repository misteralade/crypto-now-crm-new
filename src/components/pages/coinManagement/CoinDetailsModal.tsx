...
import type { SearchSupportedCryptoData, AdminCryptoWalletResponsePayload } from '../../../types/response.payload.types'
...
const WalletCard = ({ wallet }: { wallet: AdminCryptoWalletResponsePayload }) => (
  <div className="bg-[#F8F8FF] rounded-xl p-3 border border-[#ECECEC]">
    <div className="flex items-center justify-between mb-1.5">
      <span className="text-[11px] font-semibold text-[#03034D] uppercase tracking-wide">{wallet.network}</span>
      <span
        className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full ${
          wallet.isActive === 'true' || wallet.isActive === true
      ...
      >
        ...
        {wallet.isActive === 'true' || wallet.isActive === true ? 'Active' : 'Inactive'}
      </span>
    </div>
    <div className="flex items-center gap-2">
      <code className="flex-1 text-[11px] text-[#667085] font-mono truncate">{wallet.walletAddress}</code>
      <button
...
const InfoRow = ({ label, value }: { label: string; value: React.ReactNode }) => (
  <div className="flex items-start justify-between gap-4 py-2.5 border-b border-[#F5F5FF] last:border-0">
    <span className="text-[12px] text-[#9A9A9A] font-medium flex-shrink-0">{label}</span>
    <span className="text-[13px] text-[#0E0F0C] font-medium text-right">{value}</span>
  </div>
)

const SectionHeader = ({ label }: { label: string }) => (
  <h4 className="text-[11px] font-semibold text-[#9A9A9A] uppercase tracking-widest mb-3 mt-5 first:mt-0">{label}</h4>
)
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-[#ECECEC] bg-white sticky top-0">
          {loading ? (
            ...
            <div className="w-10 h-10 rounded-full bg-[#D3D4F8] flex items-center justify-center">
              <span className="text-[14px] font-bold text-[#03034D]">{coin?.symbol?.charAt(0)}</span>
            </div>
          )}
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <>
                <div className="flex items-center gap-2">
                  <h2 className="text-[17px] font-bold text-[#0E0F0C] truncate">{coin?.name}</h2>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span
                    className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-0.5 rounded-full ${
                      coin?.isActive ? 'bg-[#ECFDF3] text-[#037847]' : 'bg-[#F2F4F7] text-[#6C778B]'
                    }`}
                  >
                    <span className={`w-1.5 h-1.5 rounded-full ${coin?.isActive ? 'bg-[#14BA6D]' : 'bg-[#6C778B]'}`} />
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {loading && (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <span className="text-[13px] text-[#9A9A9A]">Loading coin details…</span>
            </div>
          )}
          {!loading && !coin && (
            <div className="flex flex-col items-center justify-center py-16 gap-2">
              <span className="text-[14px] text-[#9A9A9A]">Failed to load coin details.</span>
            </div>
          )}
          {!loading && coin && (<>
          {/* Rates */}
          ...
          <div className="grid grid-cols-2 gap-3 mb-1">
            <div className="bg-[#F5F5FF] rounded-xl p-3">
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span className="text-[11px] font-semibold text-[#9A9A9A] uppercase tracking-wide">Buy Rate</span>
              </div>
              <span className="text-[20px] font-bold text-[#03034D]">
                {coin.buyRate ? `₦${Number(coin.buyRate).toLocaleString()}` : '—'}
              </span>
            </div>
            <div className="bg-[#F5F5FF] rounded-xl p-3">
              <div className="flex items-center gap-1.5 mb-1">
                <TrendingDown className="w-3.5 h-3.5 text-[#03034D]" />
                <span className="text-[11px] font-semibold text-[#9A9A9A] uppercase tracking-wide">Sell Rate</span>
              </div>
              <span className="text-[20px] font-bold text-[#03034D]">
                {coin.sellRate ? `₦${Number(coin.sellRate).toLocaleString()}` : '—'}
              </span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#ECECEC] px-4">
            <InfoRow
              ...
              value={coin.minTransactionLimit ? `$${Number(coin.minTransactionLimit).toLocaleString()}` : '—'}
            />
            <InfoRow
              label="Authenticated — Max"
              value={coin.maxTransactionLimit ? `$${Number(coin.maxTransactionLimit).toLocaleString()}` : '—'}
            />
            <InfoRow
              label="Anonymous — Min"
              value={coin.minTradeAmountForAnonymous ? `$${Number(coin.minTradeAmountForAnonymous).toLocaleString()}` : '—'}
            />
            <InfoRow
              label="Anonymous — Max"
              value={coin.maxTradeAmountForAnonymous ? `$${Number(coin.maxTradeAmountForAnonymous).toLocaleString()}` : '—'}
            />
          </div>
          ...
          {coin.networks && coin.networks.length > 0 ? (
            ...
            <p className="text-[13px] text-[#9A9A9A]">No networks configured.</p>
          )}
          ...
          {Object.keys(walletsByNetwork).length > 0 ? (
            <div className="space-y-4">
              {Object.entries(walletsByNetwork).map(([network, wallets]) => (
                <div key={network}>
                  <div className="flex items-center gap-2 mb-2">
                    <Wallet className="w-3.5 h-3.5 text-[#948EEE]" />
                    <span className="text-[12px] font-semibold text-[#03034D]">{network}</span>
                    <span className="text-[11px] text-[#9A9A9A]">({wallets.length} address{wallets.length !== 1 ? 'es' : ''})</span>
                  </div>
                  <div className="space-y-2">
                    {wallets.map((w) => (
            ...
            <p className="text-[13px] text-[#9A9A9A]">No admin wallets configured.</p>
          )}
          ...
          <div className="bg-white rounded-xl border border-[#ECECEC] px-4">
            <InfoRow label="Coin ID" value={<code className="text-[11px] font-mono">{coin.id}</code>} />
            <InfoRow
              label="Added"
              value={coin.createdAt ? new Date(coin.createdAt).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) : '—'}
            />
          </div>
